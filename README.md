...
3. Keep the main session orchestration-only and allow it to delegate matching work to specialist workers by default.
4. Allow workflow progression through:
   - Ideation
   - Product Specification
   - Architecture
   - Implementation Planning
   - Implementation
5. Only intervene when:
...
2. Let the main session delegate to the relevant specialist worker by default.
3. Run a workflow from `.ai/workflows/` (feature, bugfix, refactor, release).
4. Start from templates in `.ai/templates/` for consistent specs, reviews, and tests.
5. Use `examples/` as reference implementations of multi-agent handoffs.
...
When generated/runtime-native workers are present, they are the default specialist workers for matching tasks.
...
When present, generated Codex workers are the preferred execution path for matching specialist work and the Codex main session must remain orchestration-only.

## Claude Runtime Usage
Generated Claude adapters are project-level subagent artifacts under `.claude/agents/*.md`.  
Canonical process/role contracts remain in `.ai/*`.
When present, generated Claude workers are the preferred execution path for matching specialist work and the Claude main session must remain orchestration-only.
...
When generated OpenCode workers are present in a consumer project, they are the default specialist workers for matching tasks and the primary OpenCode agent must remain orchestration-only.
...
Runtime metadata may label the routing shape as `auto`, `sequential`, `targeted`, or `delegated`, but that metadata is secondary.
...
Codex, Claude, and OpenCode routing must still be capability-aware. Runtime metadata or prompt-body `Execution mode: ...` is routing input only; it does not automatically spawn agents and does not decide whether delegation is required.
...
- Generated/native workers are expected delegation targets when present.
...
- `qa.md` (`tester`): test strategy and risk-based validation.
- `code-review.md` (`reviewer`): rigorous review gate.
...
## Examples
- `laravel-project`: backend feature collaboration (architect + backend + laravel + tester + reviewer).
- `vue-project`: frontend feature collaboration (architect + frontend + vue + tester).
- `react-project`: frontend feature collaboration (architect + frontend + react + tester).
- `fastapi-project`: API feature collaboration (architect + backend + fastapi + tester + reviewer).
